...
from ppci import ir
from ppci.binutils.assembler import BaseAssembler
from ppci.arch import Architecture
from ppci.arch.arch_info import ArchInfo, TypeInfo
from ppci.arch.data_instructions import DByte, DZero, data_isa
from ppci.arch.generic_instructions import Label, RegisterUseDef
from ppci.arch.stack import FramePointerLocation, StackLocation
from . import instructions
from .asm_printer import RiscObfAsmPrinter
...
class RiscObfAssembler(BaseAssembler):
    def __init__(self):
        super().__init__()
        self.lit_pool = []
        self.lit_counter = 0
...
class RiscObfArch(Architecture):
    name = "riscobf"

    def __init__(self):
        super().__init__()
        self.regclass = register_classes
        ...
        self.gdb_pc = PC
        self.asm_printer = RiscObfAsmPrinter()
        self.assembler = RiscObfAssembler()
    ...
    def branch(self, reg, lab):
        if isinstance(lab, RiscObfRegister):
    ...
    def gen_riscobf_memcpy(self, dst, src, tmp, size):
    ...
    def gen_call(self, frame, label, args, rv):
        ...
        for arg_loc, arg2 in zip(arg_locs, args):
            arg = arg2[1]
            if isinstance(arg_loc, RiscObfRegister):
                yield self.move(arg_loc, arg)
            elif isinstance(arg_loc, StackLocation):
                stack_size += arg_loc.size
                if isinstance(arg, RiscObfRegister):
                    yield Sw(arg, arg_loc.offset, SP)
                elif isinstance(arg, StackLocation):
                    p1 = frame.new_reg(RiscObfRegister)
                    p2 = frame.new_reg(RiscObfRegister)
                    v3 = frame.new_reg(RiscObfRegister)
                    ...
                    yield from self.gen_riscobf_memcpy(p1, p2, v3, arg.size)
